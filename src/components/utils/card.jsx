...
export function Card({ list, setNotice, boardId }) {
    ...
    return (
        list.cards.map((card) => (
            <div id={card._id} key={card._id} disabled={card.complete} className="w-full h-auto flex flex-row items-center justify-start gap-2 border border-gray-400 rounded-md p-2 bg-gray-400/50 dark:bg-gray-950/50 shadow-sm">
                <input onChange={() => handleCheck(card._id, card.complete)} type="checkbox" checked={card.complete} />
                <textarea rows="1" onClick={() => setIsEditing(card._id)} className={`${card.complete ? "line-through" : ""} text-justify w-full p-2`} defaultValue={card.text || "Null"} />
                {isEditing === card._id ? (
