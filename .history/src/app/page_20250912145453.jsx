...
export default function Home() {
  ...
  const handleNewBoard = (e) => {
    e.preventDefault();
    const newBoard = { title: title.current.va, color: color.current.value, description: description.current.value };
        ...
        {boardBox && (
          <form onSubmit={ handleNewBoard } className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-[#b32509] p-6 rounded-md shadow-lg">
            <div className="flex flex-row items-center justify-between gap-2">
              <input
                type="text" ref={title} onChange={ (e) => title.current = e.target.value }
