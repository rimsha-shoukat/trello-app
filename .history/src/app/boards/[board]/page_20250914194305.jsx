...
const addNewCard = ({listId, board}) => {
  const newCard = { id: Date.now().toString(), title: 'New Card', description: '', comments: [] };
  ...
  const newBoardList = board.map(b => {
    if (b.id === board.id) {
      return { ...b, lists: updatedBoards };
    }
    return b;
  });
}
            ...
            <div
              ...
              {board.lists && Array.isArray(board.lists) && board.lists.map(list => (
                <div key={list.id} style={{ backgroundColor: list.bg }} className={`w-[24rem] h-auto p-4 border-none flex flex-col items-center justify-center overflow-hidden rounded-md shadow-md`}>
                  ...
                  <button onClick={ () => addNewCard(list.id, board) } className="w-[100%] hover:scale-105 rounded-md cursor-pointer font-semibold text-[#b32509] py-[0.3rem] bg-gray-400 hover:bg-gray-300/60 shadow-md transition-all duration-300">+ card</button>
                </div>
              ))
              }
            </div>
