...
export default function cardArea ({ cards, board, activeList, boardList, setBoardList }) {
    ...
    const editCard = (Id) => {
        const updatedCards = cards.map(card => {
            if (card.id === Id) {
                return { ...card, description: updatedText[Id] || card.description };
            }
            return card;
        });
        Updates(updatedCards);
               ...
               <div key={card.id} className="relative w-[100%]">
                {
                 ...
                 <MdOutlineDelete style={{ color: card.text }} onClick={ () => deleteCard(card.id)} className="hover:scale-105 text-[1.5rem] cursor-pointer transition-opacity duration-300"/>
                    ...
                    <section style={{backgroundColor: card.bg}} className="w-[100%] flex flex-row items-s justify-start gap-2 h-auto p-2 border-0 rounded-md shadow-md">
                        ...
                        rows="1"
                        value={editMode === card.id ? (updatedText[card.id] || '') : card.description}
