...
export default function cardDialogBox({ newCard, setNewCard, board, activeList, boardList, setBoardList }) {
    ...
    return (
        <>
            {
                newCard && (
                    <form onSubmit={handleNewCard} className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-[#2e1d06] p-6 rounded-md shadow-lg">
                        <h1 className="text-[1rem] font-semibold">Add new card</h1>
                        <textarea type="text" rows='3' value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Enter card details..." className="mt-2 p-2 rounded-md w-[12rem] border-2 border-gray-800/50 focus:outline-none focus:ring-0" />
                        ...
                        <div className="flex flex-row w-[100%] items-center justify-between mt-2">
                            <button
                                ...
                                className="px-4 py-[0.35rem] bg-[#2e1d06] hover:shadow-sm hover:scale-95 hover:shadow-gray-300 rounded-md cursor-pointer font-semibold shadow-sm transition-all duration-300"
                            >
                                Cancel
                            </button>
                            <button type="submit"
                                className="px-4 py-[0.35rem] bg-[#2e1d06] hover:shadow-sm hover:scale-105 hover:shadow-gray-300 rounded-md cursor-pointer font-semibold text-white shadow-md transition-all duration-300"
                            >
                                Create
                            </button>
                        </div>
