...
export default function cardArea ({ cards, board, activeList, boardList, setBoardList }) {
    ...
    setEditMode(id);
    setUpdatedTexts(prev => ({ ...prev, [id]: currentDescription }));
  };

    const editCard = (Id) => {
        const updatedCards = cards.map(card => {
            if (card.id === Id) {
                return { ...card, description: updatedTexts[Id] || card.description };
            }
            return card;
        });
        Updates(updatedCards);
    ...
    setUpdatedTexts(prev => ({ ...prev, [id]: value }));
  };
               ...
               <div key={card.id} className="relative w-[100%]">
                {
                 ...
                 <TbCancel style={{ color: card.text }} onClick={ () => setEditMode(null)} className="hover:scale-105 text-[1.5rem] cursor-pointer transition-opacity duration-300" />
                 <VscSaveAs  style={{ color: card.text }} onClick={ () => editCard(card.id)} className="hover:scale-105 text-[1.5rem] cursor-pointer transition-opacity duration-300"/>
                 </div>
                    ) : (
                        <div className="absolute top-2 right-2 flex flex-row gap-2">
                 <CiEdit style={{ color: card.text }}  onClick={() => handleEditCard(card.id, card.description)} className="hover:scale-105 text-[1.5rem] cursor-pointer transition-opacity duration-300"/>
                 <MdOutlineDelete style={{ color: card.text }} onClick={ () => deleteCard(card.id)} className="hover:scale-105 text-[1.5rem] cursor-pointer transition-opacity duration-300"/>
                    ...
                    <section key={card.id} style={{backgroundColor: card.bg}} className="w-[100%] flex flex-row items-s justify-start gap-2 h-auto p-2 border-0 rounded-md shadow-md">
                        ...
                        <textarea readOnly={editMode !== card.id}
              onChange={(e) => handleTextChange(card.id, e.target.value)}
              rows="1"
              value={editMode === card.id ? (updatedTexts[card.id] || '') : card.description}
              style={{ color: card.text }} style={{color: card.text}} className="text-[1rem] text-justify w-auto h-auto leading-6" />
                    </section>
                </div>
                ))
            }
