...
export default function cardArea ({ cards, board, activeList, boardList, setBoardList }) {
    ...
    const editCard = (Id) => {
        ...
        const updatedCards = cards.map(card => {
            if (card.id === Id) {
                return card.description = <textArea readOnly={!editMode} type="text" rows='1' value={card.description} style={{color: card.text}} className="text-[1rem] text-justify leading-6"/>;
            }
        });
        Updates(updatedCards.filter(card => card !== undefined));
    }
