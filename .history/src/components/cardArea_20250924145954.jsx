      ...
      {cards.map((card) => {
        ...
        return (
          <div key={card.id} className="relative w-full group mb-3">
            ...
            {/* Card Body */}
            <section 
            data-swapy-item={card.id}  // Always present: Makes the card draggable (unique ID)
              draggable={true}
              style={{ backgroundColor: cardBg }}
              className={`
                w-full flex items-start justify-start gap-3 h-auto p-4 border-2 border-white/20 rounded-xl shadow-md 
            ...
            >
              ...
              {!isEditing && (
                <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/0 via-purple-500/0 to-pink-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-xl p