            ...
            <div key={board.id} className="relative w-[20rem] h-[12rem]">
              {
                editMode ? (
                 ...
                 <VscSaveAs  style={{ color: card.text }} onClick={ () => editList(l.id)} className="hover:scale-105 text-[1.5rem] cursor-pointer transition-opacity duration-300"/>
                  ...
                  <div className="absolute top-2 right-2 flex flex-row gap-2">
              <CiEdit style={{ color: board.text }} onClick={()=> handleEditBoard(board)} className="text-[1.5rem] hover:scale-105 cursor-pointer transition-opacity duration-300"/>
              ...
              <Link key={board.id} href={`/boards/${board.id}`}>
                <div style={{ backgroundColor: board.bg }} className={`w-[20rem] h-[12rem] p-4 border-none flex items-center justify-center overflow-hidden rounded-md shadow-md`}>
                  <input readOnly={!editMode} onChange={ (e) => setUpdatedText(e.target.value) } type="text" value={board.title} style={{ color: board.text }} className="text-[2rem] leading-7"/>
                </div>
              </Link>
            </div>
            ))}
