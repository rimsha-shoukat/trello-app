...
export default function listArea({ board, setNewCard, setActiveList, boardList, setBoardList }) {
  ...
  const [updatedText, setUpdatedText] = useState({});
   ...
   const handleDragEnd = (e) => {
    const draggedItemId = e.detail?.item?.dataset?.swapyItem;
    const dropSlotId = e.detail?.slot?.dataset?.swapySlot;
    if (!draggedItemId || !dropSlotId) return;
    // Basic move logic (expand as needed)
    let updatedLists = [...list];
    let draggedCard = null;
    let originalListId = null;
    // Find and remove card from original list
    ...
    if (draggedCard) {
      // Add to target list
      const targetIdx = updatedLists.findIndex(l => l.id == dropSlotId);
      if (targetIdx > -1) {
        updatedLists[targetIdx].cards = [...(updatedLists[targetIdx].cards || []), draggedCard];
      }
      const updatedBoards = boardList.map(b => b.id === board.id ? { ...b, lists: updatedLists } : b);
  ...
  useEffect(() => {
    if (container.current) {
      // Simple delay to wait for CardArea to render cards
      const timer = setTimeout(() => {
        swapy.current = createSwapy(container.current);
        swapy.current.addEventListener('dragend', handleDragEnd);  // Or .on('dragend', ...) if that's the API
      }, 50);  // 50ms is enough for dynamic load
       return () => {
        clearTimeout(timer);
        swapy.current?.destroy();
      };
    }
  }, [list]);  // Re-init if lists change
