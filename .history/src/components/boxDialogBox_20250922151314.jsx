...
export default function boxDialogBox({ boardBox, setBoardBox, boardList, setBoardList }) {
    ...
    return (
        <>
            {boardBox && (
                <form onSubmit={handleNewBoard} className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-[#333231] p-6 rounded-md shadow-lg z-10">
                    <input
                        type="text" value={title} onChange={(e) => setTitle(e.target.value)}
                        className="border-2 p-2 rounded-md w-[12rem] border-gray-300/50 focus:outline-none focus:ring-0"
