...
export default function cardDialogBox({ newCard, setNewCard, board, currentList }) {
    ...
    const handleNewCard = (e) => {
        ...
        const updatedCards = board.map(b => {
            if (b.id === currentList) {
                return { ...b, cards: Array.isArray(b.cards) ? [...b.cards, newCard] : [newCard] };
            }
            return b;
        });
        localStorage.setItem('boards', JSON.stringify(updatedCards));
        setDescription('');
        setBg('#616060');
        setText('#f5f2f2');
        setNewCard(false);
