    ...
    return (
        <>
            {cards && Array.isArray(cards) && cards.map((card) => (
               <div key={card.id} className="relative w-[20rem] h-[12rem]">
                 <CiEdit style={{ color: card.text }} className="absolute text-[1.5rem] cursor-pointer top-2 right-2 opacity-0 hover:opacity-100 transition-opacity duration-300"/>
