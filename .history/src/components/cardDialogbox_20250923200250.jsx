...
export default function cardDialogBox({ newCard, setNewCard, board, activeList, boardList, setBoardList }) {
  ...
  return (
    <>
      {/* Backdrop */}
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 inset-0 bg-black/50 backdrop-blur-sm z-20 flex items-center justify-center p-4" onClick={() => setNewCard(false)}>
        {/* Modal Content */}
        <form 
          onSubmit={handleNewCard} 
          onClick={(e) => e.stopPropagation()}
          className="bg-white/80 backdrop-blur-md p-6 rounded-2xl shadow-2xl border border-white/20 w-full max-w-sm animate-in fade-in zoom-in duration-300"
        >
          ...
          <div className="mb-4">
            ...
            <textarea
              ...
              className="w-full  border-2 p-3 rounded-xl border-gray-200/50 focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/20 bg-white/50 backdrop-blur-sm transition-all duration-200 shadow-sm hover:shadow-md resize-none"
