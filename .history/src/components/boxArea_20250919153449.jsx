      ...
      <h1 className={`${greeting ? 'block' : 'hidden'} font-bold text-[3rem]`}>Welcome to Trello!</h1>
      {
        boardList && Array.isArray(boardList) && boardList.length === 0 ? (
          ...
          <section style={{
            ...
            {boardList.map(board => (
            <div key={board.id} className="relative w-[2rem] h-[12rem]">
              {
                editMode ? (
              ...
              <Link key={board.id} href={`/boards/${board.id}`}>
                <div style={{ backgroundColor: board.bg }} className={`w-[100%] h-[100%] p-4 border-none flex items-center justify-center overflow-hidden rounded-md shadow-md`}>
