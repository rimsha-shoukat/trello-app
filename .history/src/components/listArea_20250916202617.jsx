        ...
        <>
            <section
                className="mt-6 w-[100%] h-auto">
                    <h1 className="text-[2.2rem] px-[2rem] text-[#2e1d06] font-bold">{board?.title}</h1>
                {list && Array.isArray(list) && list.length === 0 ? (
                    <h2 className="text-[1.2rem] text-[#2e1d06]">No lists added yet!</h2>
                ) : (
                    <>
                        <div
                            style={{
                            ...
                            {list && Array.isArray(list) && list.map(l => (
                                <div key={l.id} style={{ backgroundColor: l.bg }} className={`w-[24rem] h-auto p-4 border-none flex flex-col gap-4 items-center justify-center overflow-hidden rounded-md shadow-md`}>
                                    ...
                                    <button onClick={() => {setNewCard(!newCard); setActiveList(l.id);} } className="w-[100%] hover:scale-105 rounded-md cursor-pointer font-semibold text-[#2e1d06] py-[0.3rem] bg-gray-400 hover:bg-gray-300/60 shadow-md transition-all duration-300">+ card</button>
                                </div>
                            ))
                            }
                        </div>
