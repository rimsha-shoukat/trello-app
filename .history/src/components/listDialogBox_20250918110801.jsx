...
export default function listDialogBox({ listBox, setListBox, boardId, boardList , setBoardList }) {
    ...
    const handleNewList = (e) => {
        e.preventDefault();
        const newList = { id: Date.now().toString(), title: title, bg: bg, text: text, cards: [] };
    ...
    return (
        <>
            {listBox && (
                <form onSubmit={handleNewList} className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-[#333231] p-6 rounded-md shadow-lg z-10">
                    <input
                        type="text" value={title} onChange={(e) => setTitle(e.target.value)}
                        className="border-2 p-2 rounded-md w-[12rem] border-gray-200/50 focus:outline-none focus:ring-0"
                    ...
                    <div className="w-[100%] mt-4 flex flex-row justify-between items-center">
                        <button
                            ...
                            className="px-4 py-[0.35rem] bg-[#333231] hover:shadow-sm hover:scale-95 hover:shadow-gray-300 rounded-md cursor-pointer font-semibold shadow-sm transition-all duration-300"
                        >
                            Cancel
                        </button>
                        <button type="submit"
                            className="px-4 py-[0.35rem] bg-[#333231] hover:shadow-sm hover:scale-105 hover:shadow-gray-300 rounded-md cursor-pointer font-semibold text-white shadow-md transition-all duration-300"
                        >
                            Create
                        </button>
                    </div>
